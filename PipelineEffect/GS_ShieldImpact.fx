...
GetPixelColor( float4 iColor, float2 iTexCoord, float iDist, float3 iPos, float3 iView)
{
	...
	color.a = lerp( shieldMinAlpha, 0.f, impactDistVertex/(g_InternalHitRadius*0.08));// Transparency decreases over distance from impact position.
	...
	inversePercent *= 5.0;// increase opacity of noise
	...
	float dissipationTime = g_Time * 2;// Scale time
